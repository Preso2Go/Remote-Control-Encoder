...
function initializeimagevent(imgid, delayms, pictype){
	...
	var url="http://" + getCookie("ip") + "?action=getvisualelement&actiondetail="+ pictype +"&user=presentations2go&password=" + password;
	$(imgid).attr("src", url);

	return setInterval(function(){
		var d = new Date();
		var url="http://" + getCookie("ip") + "?action=getvisualelement&actiondetail="+ pictype +"&user=presentations2go&password=" + password + "&hour=" + d.getHours() + "&sec=" + d.getSeconds() + "&milisec=" + d.getMilliseconds();
		$(imgid).attr("src", url);
	}, delayms);
}
